...
bool isEllipse(vector<cv::Point> contour){
	...
	perimeterActual = cvContourPerimeter(&contour);

	if (areaActual < 10) {
		areaIdeal = radii.area()*3.141593 / 4;
		//Check if ellipse
		if (abs(areaIdeal - areaActual) / areaIdeal > 0.0005) {
			return false;
		}
		else {
			return true;
...
FeatureFinder::FeatureFinder(string boardFile, string cameraFile){
	...
	if (!bf.isOpened()){
		...
		if (shapeType == 2){
			bf["altId"] >> altId;
			bf["altShape"] >> tempAlt;
			altShape.assign(tempAlt.col(0).datastart, tempAlt.col(0).dataend);
		}
		bf.release();
			...
			if (circles.size() != nlocs) {
				...
				if (markerIds.size() > 0){
					...
					for (int i = 0; i < diamondCorners.size(); i++){
						if (diamondIds.at(i) == altId){
							found = true;
							targetshape = altShape;
							cameraPoints = diamondCorners.at(i);
							break;
						}
					}
				}
