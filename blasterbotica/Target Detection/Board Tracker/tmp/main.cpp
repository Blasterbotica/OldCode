...
int main(int argc, char* argv[]) {
	...
	string boardFile = "hybrid_target.xml";
	char* comport = "/dev/ttyUSB";
	//////////////////////////////////////////
	...
	cv::FileStorage fs(cameraFile, FileStorage::READ);
	if (!fs.isOpened()){
		cout << "Could not read camera parameters\n";
		exit(1);
	}
	...
	//Starts feature finder with square target
	...
	while (running) {
		...
		if (!lost){ // z=0 means target inside camera, impossible
			...
			cv::circle(videoFrame, piccenter.at(0), 5, Scalar(255, 0, 0), 1);
			...
			if (superserial.IsConnected()){
				...
				*/
				...
				//Bounds theta and phi to -180, 180 (-90, 90 -> PI / 2)
				if (theta < -PI) {
					theta = -PI;
				}
				else if (theta > PI){
